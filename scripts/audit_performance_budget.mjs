 ...
 *   2. **Un seul WebSocket par visiteur** : plus aucun `createSafeChannel` sur le
 *      chemin public (tout passe par `subscribeTable`, canal partagé) ;
 *   3. **Zéro requête publique nominale** : quand le serveur a fourni le contenu,
 *      le client ne rejoue pas de requête Supabase ;
 *   4. **Publication visible des deux côtés** : la revalidation couvre la voie
...
/* 1. Un seul WebSocket : plus de createSafeChannel sur le chemin public. */
const legacyChannels = publicFiles
    .filter((file) => readFileSync(file, 'utf8').includes('createSafeChannel'))
    .map((file) => relative(ROOT, file));

const sharedChannelHelper = readFileSync(REALTIME_LIB, 'utf8').includes('export function subscribeTable');
const previewSkipsRealtime = readFileSync(REALTIME_LIB, 'utf8').includes('isPreviewFrame()');
...
const checks = [
    {
        label: 'Un seul WebSocket par visiteur (canal partagé)',
        ok: legacyChannels.length === 0 && sharedChannelHelper,
        detail:
            legacyChannels.length === 0
                ? 'Aucun `createSafeChannel` sur le chemin public ; tout passe par `subscribeTable`.'
