...
| Copie visible sur `/en` | HUD du plan 3D public et liens de la fiche coach sortis du français en dur (catalogues `campus3dViewer` et `team.coach*`, FR **et** EN) ; garde-fou [`CampusViewerHUD.test.tsx`](src/components/3d/ui/CampusViewerHUD.test.tsx:1) — l'EN est servi, le FR ne l'est jamais sur une page anglaise. Reste assumé : chrome des éditeurs studio 3D et Cockpit (FR par conception, non routé en EN) et noms propres (Allociné, logos) que le compteur compte à tort — [`revue-traductions-manquantes.md`](plans/revue-traductions-manquantes.md:1) |
