...
interface StagesPageEditorProps {
  ...
  handleUpdateStageItem: (index: number, updates: any) => void;
    ...
    <div className="space-y-6">
      <div className="bg-[#0D0D12] border border-white/10 rounded-xl p-6 space-y-4">
        ...
        <div className="space-y-4">
          {(formData.sections_data?.stages_catalogue?.items || []).map((stg: any, idx: number) => (
            <div
              key={stg.id || idx}
            ...
            >
              ...
              <div>
                ...
                <textarea
                  rows={2}
                  value={stg.desc || ''}
                  onChange={(e) => handleUpdateStageItem(idx, { desc: e.target.value })}
                  className="w-full bg-black/60 border border-white/20 rounded px-2.5 py-1.5 text-xs text-white"
                />
              </div>
            </div>
